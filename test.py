...
# 페이지 설정
# ──────────────────────────────────────────────
st.set_page_config(page_title="지수·로그 교점 찾기",
                   layout="centered")
...
아래 입력창에 **a, b, c, p, d, e, f** 값을 넣고 **🔍 교점 찾기**를 누르세요.  
제약 조건: \(a>0,\;a\neq1,\;d>0,\;d\neq1,\;x+e>0\)
""")
...
with col_log:
    ...
    d = st.number_input("d (밑, >0 & ≠1)", value=10.0, format="%.6f")
...
def g(x: float) -> float:
    """지수 - 로그  (g(x)=0 가 교점)"""
    return a ** (x + b) + c - (p * np.log(x + e) / np.log(d) + f)
...
if btn:
    ...
    if not roots:
        ...
        table = [{"#": i+1,
                  "x": r,
                  "y": a**(r+b)+c} for i, r in enumerate(roots)]
        st.table(table)

        st.markdown(
            """
- 차이 함수 \(g(x)=a^{x+b}+c -\\bigl(p\\log_{d}(x+e)+f\\bigr)\)를 정의하고  
  **부호 변화 구간**을 찾은 뒤 `scipy.optimize.brentq`로 근을 구했습니다.
- 근 중복은 |Δx| < 1 e-7 범위에서 제거했습니다.
""")
        ...
        mask = x_dense + e > 0
        x_log = x_dense[mask]
        y_log = p * np.log(x_log + e) / np.log(d) + f
